...
class KernelPointFCNN:
    ...
    def __init__(self, flat_inputs, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.onehot_labels = [tf.cast(tf.one_hot(self.labels, 9)[:,1:], tf.int32)]
            tmp_layer = 0
        ...
        with tf.variable_scope('KernelPointNetwork'):
            ...
            output_features = tf.concat((output_features, tf.tile(self.inputs['features'][:,4:5],[1,4])), axis=1)
            if config.network_model in ["multi_segmentation", 'multi_cloud_segmentation']:
        ...
        with tf.variable_scope('loss'):
            ...
            # Add regularization
            self.loss = self.regularization_losses() + self.output_loss + self.h_loss + se_loss
