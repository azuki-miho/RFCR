...
def assemble_FCNN_blocks(inputs, config, dropout_prob):
    ...
    features = F[-1]
    supervised_features = []
    tmp_features = unary_block(None, None, features, None, 13,  config, dropout_prob < 0.99, activation_fn=False)
    ...
    for block_i, block in enumerate(config.architecture[start_i:]):
        ...
        if 'upsample' in block:
            ...
            supervised_features.append(tmp_features)
            if num_loss < 2.5:
                target_features = tf.cast(tf.greater_equal(features, 0.), tf.int32)
                if self_enhance_loss is None:
                    self_enhance_loss = tf.losses.sigmoid_cross_entropy(target_features, features)
                    num_loss += 1
                else:
                    self_enhance_loss += tf.losses.sigmoid_cross_entropy(target_features, features)
                    num_loss += 1
...
def segmentation_loss(logits, inputs, batch_average=False):
    ...
    # Exclusive Labels cross entropy on each point
    class_weight = [27862126, 24004020, 45616923,  3964721,  3664208,  3177430, 10098833, 7981518,  5903320,  9238512,   881504,  2276780, 21159594,]
