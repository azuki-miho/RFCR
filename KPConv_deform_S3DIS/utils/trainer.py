        ...
        while self.training_epoch < model.config.max_epoch:
            ...
            try:
                ...
                if debug_NaN:
                    ...
                    # Run normal
                    if self.training_epoch < 30:
                        _, L_out, L_reg, L_p, L_h, probs, labels, acc = self.sess.run(ops, {model.dropout_prob: 0.5, model.se_weight: 0.})
                    else:
                        _, L_out, L_reg, L_p, L_h, probs, labels, acc = self.sess.run(ops, {model.dropout_prob: 0.5, model.se_weight: 1.})

                t += [time.time()]

                #
                labels_onehot = np.eye(13)[labels]
    ...
    def cloud_validation_error(self, model, dataset):
        ...
        if model.config.saving:
            ...
            for IoU in IoUs:
                line += '{:.4f} '.format(IoU)
            line = line + '\n'
            tmp_mIoU = np.mean(IoUs) * 100

            # Write in file
            if exists(test_file):
                with open(test_file, "a") as text_file:
                    text_file.write(line)
                    text_file.write('{:s} mean IoU = {:.4f}%\n'.format(model.config.dataset, tmp_mIoU))
            else:
                with open(test_file, "w") as text_file:
                    text_file.write(line)
                    text_file.write('{:s} mean IoU = {:.4f}%\n'.format(model.config.dataset, tmp_mIoU))
