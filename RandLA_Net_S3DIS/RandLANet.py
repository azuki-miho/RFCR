"""
Modified from RandLA-Net:https://github.com/QingyongHu/RandLA-Net
Author: Jingyu Gong
Date: September 2021
"""
...
class Network:
    def __init__(self, dataset, config):
        ...
        if self.config.saving:
            ...
            self.saving_path = time.strftime('test/Log_%Y-%m-%d_%H-%M-%S', time.gmtime())
            makedirs(self.saving_path) if not exists(self.saving_path) else None
        ...
        with tf.variable_scope('layers'):
            self.logits, self.h_loss, self.se_loss = self.inference(self.inputs, self.is_training)
        ...
        with tf.variable_scope('loss'):
            ...
            self.se_weight = tf.placeholder(tf.float32, name='se_weight')
            self.loss = self.get_loss(valid_logits, valid_labels, self.class_weights) + self.se_loss * self.se_weight + self.h_loss
    ...
    def inference(self, inputs, is_training):
        ...
        #
        multihot_labels = [tf.cast(tf.one_hot(inputs['labels'], 13), tf.int32)]
        ...
        for i in range(self.config.num_layers):
            ...
            #label sample
            tmp_multihot_label = tf.reduce_max(self.gather_neighbour(multihot_labels[i], inputs['neigh_idx'][i]), axis=2)
            tmp_multihot_label = tf.reduce_max(self.gather_neighbour(tmp_multihot_label, inputs['neigh_idx'][i]), axis=2)
            f_sampled_i = self.random_sample(f_encoder_i, inputs['sub_idx'][i])
            #label sample
            tmp_multihot_label = tf.squeeze(self.random_sample(tf.expand_dims(tmp_multihot_label, axis=2), inputs['sub_idx'][i]), [2])
            feature = f_sampled_i
            if i == 0:
        ...
        # ###########################Decoder############################
        supervised_features = []
        tmp_features = helper_tf_util.conv2d(feature, 13, [1, 1], "sf{}".format(0), [1, 1], 'VALID', True, is_training, activation_fn=None)
        supervised_features.append(tf.squeeze(tmp_features, [2]))
        ...
        for j in range(self.config.num_layers):
            ...
            if j < 4:
                #se
                se_features = helper_tf_util.conv2d(f_decoder_i, 10, [1, 1], "se{}".format(j), [1, 1], 'VALID', True, is_training, activation_fn=None)
                target_se_features = tf.cast(tf.greater_equal(se_features, 0.), tf.int32)
                if self_enhance_loss is None:
                    self_enhance_loss = tf.reduce_mean(tf.losses.sigmoid_cross_entropy(target_se_features, se_features))
                    num_loss += 1
                else:
                    self_enhance_loss += tf.reduce_mean(tf.losses.sigmoid_cross_entropy(target_se_features, se_features))
                    num_loss += 1
            tmp_features = helper_tf_util.conv2d(feature, 13, [1, 1], "sf{}".format(j+1), [1, 1], 'VALID', True, is_training, activation_fn=None)
            supervised_features.append(tf.squeeze(tmp_features, [2]))
        # ###########################Decoder############################
        ...
        h_loss = tf.reduce_mean(tf.losses.sigmoid_cross_entropy(multihot_labels[5], supervised_features[0]))
        h_loss += tf.reduce_mean(tf.losses.sigmoid_cross_entropy(multihot_labels[4], supervised_features[1]))
        h_loss += tf.reduce_mean(tf.losses.sigmoid_cross_entropy(multihot_labels[3], supervised_features[2]))
        h_loss += tf.reduce_mean(tf.losses.sigmoid_cross_entropy(multihot_labels[2], supervised_features[3]))
        h_loss /= 4.
        return f_out, h_loss, self_enhance_loss
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                if self.training_epoch < 5:
                    _, _, summary, l_out, l_h, l_se, probs, labels, acc = self.sess.run(ops, {self.is_training: True, self.se_weight: 0.})
                else:
                    _, _, summary, l_out, l_h, l_se, probs, labels, acc = self.sess.run(ops, {self.is_training: True, self.se_weight: 1.})
                self.train_writer.add_summary(summary, self.training_step)
                t_end = time.time()
                if self.training_step % 50 == 0:
                    message = 'Step {:08d} L_out={:5.3f} L_h={:5.3f} L_se={:5.3f} Acc={:4.2f} ''---{:8.2f} ms/batch'
                    log_out(message.format(self.training_step, l_out, l_h, l_se, acc, 1000 * (t_end - t_start)), self.Log_file)
                self.training_step += 1
