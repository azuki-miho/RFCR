...
def unary_block(layer_ind, inputs, features, radius, fdim, config, training, activation_fn=True):
    """
...
def simple_bibranch_block(layer_ind, inputs, features, radius, fdim, config, training):
    """
    Block performing a simple convolution
    """
    ...
    print("Here")
    # Weights
    features1 = features[:,:4]
    features2 = tf.concat([features[:,0:1],features[:,4:]], axis=1)
    with tf.variable_scope('color'):
        x1 = simple_block(layer_ind, inputs, features1, radius, fdim, config, training)
    with tf.variable_scope('xyz'):
        x2 = simple_block(layer_ind, inputs, features2, radius, fdim, config, training)
...
def resnetb_bibranch_block(layer_ind, inputs, features, radius, fdim, config, training):
    with tf.variable_scope('color'):
        x1 = resnetb_block(layer_ind, inputs, features[:,:int(features.shape[1]//2)], radius, fdim, config, training)
    with tf.variable_scope('xyz'):
        x2 = resnetb_block(layer_ind, inputs, features[:,int(features.shape[1]//2):], radius, fdim, config, training)
    x = tf.concat([x1, x2], axis=1)
    return x
...
def get_block_ops(block_name):
    ...
    if block_name == 'simple_bibranch':
        return simple_bibranch_block
    ...
    elif block_name == 'resnetb_bibranch':
        return resnetb_bibranch_block
...
def assemble_FCNN_blocks(inputs, config, dropout_prob):
    ...
    features = F[-1]
    supervised_features = []
    tmp_features = unary_block(None, None, features, None, 20, config, dropout_prob < 0.99, activation_fn=False)
    supervised_features.append(tmp_features)
    ...
    for block_i, block in enumerate(config.architecture[start_i:]):
        ...
        if 'upsample' in block:
            ...
            with tf.variable_scope('se'):
                se_features = unary_block(None, None, features, None, 20, config, dropout_prob < 0.99, activation_fn=False)
                target_se_features = tf.cast(tf.greater_equal(se_features, 0.), tf.int32)
                if self_enhance_loss is None:
                    self_enhance_loss = tf.losses.sigmoid_cross_entropy(target_se_features, se_features)
                    num_loss += 1
                else:
                    self_enhance_loss += tf.losses.sigmoid_cross_entropy(target_se_features, se_features)
                    num_loss += 1
    ...
    return features, supervised_features, self_enhance_loss
...
def h_post_loss(logits, inputs):
    tmp_softmax = tf.nn.softmax(logits, axis=1)
    tmp_onehot_labels = tf.cast(tf.one_hot(inputs['point_labels'], 21)[:,1:], tf.float32)
    region_prediction = tf.gather(tmp_softmax, inputs['pools'][0])
    region_prediction = tf.reduce_mean(region_prediction, axis=1)
    region_labels = tf.gather(tmp_onehot_labels, inputs['pools'][0])
    region_labels = tf.reduce_mean(region_labels, axis=1)
    region_ce = - region_labels * tf.log(region_prediction + 1e-6)
    region_ce = tf.reduce_mean(region_ce)
    return region_ce
...
def segmentation_loss(logits, inputs, batch_average=False):
    ...
    # Exclusive Labels cross entropy on each point
    class_weight = [65953367, 62981346, 60546606, 10509640, 7975878, 18158342, 6863033, 10358571, 10265461, 6714047, 5010580, 1069669, 1358621, 5442555, 4460781, 1404855, 738677, 789081, 653496, 826776, 8324860]
    class_weight = class_weight[1:]
    class_weight = np.array(class_weight).astype(np.float32)
    class_weight = class_weight / np.sum(class_weight)
    class_weight = 1 / np.log(1.2 + class_weight)
    class_weight = class_weight / np.min(class_weight)
    print('class weight')
    print(class_weight)
    class_weight = tf.constant(class_weight, dtype=tf.float32)
    label_weight = tf.gather(class_weight, inputs['point_labels'])
    print(tf.shape(label_weight))
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=inputs['point_labels'],
                                                                   logits=logits,
                                                                   name='xentropy')

    print(tf.shape(cross_entropy))
    cross_entropy = cross_entropy * label_weight
