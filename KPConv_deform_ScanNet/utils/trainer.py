...
class ModelTrainer:
        ...
        while self.training_epoch < model.config.max_epoch:
            ...
            try:
                ...
                if debug_NaN:
                    ...
                    # Run normal
                    _, L_out, L_reg, L_p, L_h, probs, labels, acc = self.sess.run(ops, {model.dropout_prob: 0.5})

                t += [time.time()]

                #
                acc_tmp_list.append(acc)
                labels_onehot = np.eye(21)[labels]
                labels_onehot = np.sum(labels_onehot, axis=0)
                labels_sum += labels_onehot
                #print(labels_sum)
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = 'Step {:08d} L_out={:5.3f} L_reg={:5.3f} L_p={:5.3f} L_h={:5.3f} Acc={:4.2f} ' \
                ...
                acc_list.append(np.mean(np.array(acc_tmp_list)))
                var_list.append(np.var(np.array(acc_tmp_list)))
                np.save(join(model.saving_path, "learning_curve.npy"), np.array(acc_list))
                np.save(join(model.saving_path, "var_curve.npy"), np.array(var_list))
    ...
    def segment_validation_error(self, model, dataset):
        ...
        with open(test_file, "a") as text_file:
            text_file.write('{:s} : mIoU = {:.1f}% / vote mIoU = {:.1f}%\n'.format(model.config.dataset, mIoU, mIoU2))
    ...
    def cloud_validation_error(self, model, dataset):
        ...
        with open(test_file, "a") as text_file:
            text_file.write('{:s} mean IoU = {:.1f}%\n'.format(model.config.dataset, mIoU))

        global BEST_MIOU
        if BEST_MIOU < mIoU:
            BEST_MIOU = mIoU
            self.saver.save(self.sess, join(model.saving_path, 'snapshots', 'best_snap.ckpt'))
